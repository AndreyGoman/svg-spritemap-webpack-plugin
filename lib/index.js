...
const generateStylesheets = require('./generate-stylesheets');
...
module.exports = class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            const { output: outputOptions, styles: stylesOptions, stylesheets: stylesheetsOptions } = this.options;

            this.filenames = {
                spritemap: this.options.output.filename,
                styles: this.options.styles.filename,
                stylesheets: this.options.stylesheets.filename
            };
            ...
            if (this.spritemap) {
                ...
                this.generateStylesheets(compilation);
            }
            ...
            compilation.hooks.afterHash.tap(plugin, () => {
                ...
                if ( typeof this.stylesheets !== 'undefined' && this.stylesheetsType === 'asset' ) {
                    compilation.emitAsset(stylesheetsOptions.filename, new RawSource(this.stylesheets.content))
                }
            });
    ...
    generateStylesheets(compilation) {
        const { stylesheets: stylesheetsOptions } = this.options;
        if (!stylesheetsOptions) {
            return;
        }

        const extension = path.extname(stylesheetsOptions.filename).substring(1).toLowerCase();

        this.stylesheets = generateStylesheets(this.sources);

        // Include warnings received from the stylesheets formatters
        if ( this.stylesheets.warnings ) {
            this.warnings = [...this.warnings, ...this.stylesheets.warnings];
        }

        this.stylesheetsType  = this.getStylesType(this.stylesheets.content, stylesheetsOptions.filename);
        this.writeStylesToDisk(this.stylesheets.content, this.stylesheetsType, 'stylesheets');
    }
    ...
    writeStylesToDisk(content, type, optionsType = 'styles') {
        const { [optionsType]: stylesOptions } = this.options;
